...
class Game {
  constructor() {
    ...
    this._resize();
    // UI создаётся сразу — нужен для навигации по меню до старта игры
  ...
  _bindEvents() {
    ...
    if (btnStart) btnStart.addEventListener('click', () => this.ui.showScreen('heroSelect'));

    const btnHow = document.getElementById('btnHow');
    if (btnHow) btnHow.addEventListener('click', () => this.ui.showScreen('howToPlay'));

    const btnBackMenu = document.getElementById('btnBackMenu');
    if (btnBackMenu) btnBackMenu.addEventListener('click', () => this.ui.showScreen('mainMenu'));

    const btnRestart = document.getElementById('btnRestart');
    if (btnRestart) btnRestart.addEventListener('click', () => this.ui.showScreen('heroSelect'));

    const btnMainMenu = document.getElementById('btnMainMenu');
    if (btnMainMenu) btnMainMenu.addEventListener('click', () => { this.stopLoop(); this.ui.showScreen('mainMenu'); });
    ...
    if (btnPauseMenu) btnPauseMenu.addEventListener('click', () => { this.stopLoop(); this.ui.showScreen('mainMenu'); });

    // Hero select — confirm button
    const btnHeroConfirm = document.getElementById('btnHeroConfirm');
    if (btnHeroConfirm) btnHeroConfirm.addEventListener('click', () => {
      this.ui.showScreen('difficultySelect');
    });

    const btnHeroBack = document.getElementById('btnHeroBack');
    if (btnHeroBack) btnHeroBack.addEventListener('click', () => this.ui.showScreen('mainMenu'));

    // Difficulty select — confirm button
    const btnDiffConfirm = document.getElementById('btnDiffConfirm');
    if (btnDiffConfirm) btnDiffConfirm.addEventListener('click', () => {
      const heroId = this._pendingHero || 'WARRIOR';
      const diffId = this._pendingDiff || 'NORMAL';
      this.startGame(heroId, diffId);
    });

    const btnDiffBack = document.getElementById('btnDiffBack');
    if (btnDiffBack) btnDiffBack.addEventListener('click', () => this.ui.showScreen('heroSelect'));
  ...
  // ---- INIT ----
  startGame(heroId, difficultyId) {
    this._selectedHero = heroId || 'WARRIOR';
    this._selectedDifficulty = difficultyId || 'NORMAL';
    const self = this;
    SpriteLoader.load();
  ...
  _startGameAfterSprites() {
    ...
    // Apply hero bonuses
    const heroData = CONFIG.HEROES[this._selectedHero] || CONFIG.HEROES.WARRIOR;
    this._applyHero(heroData);

    // Apply difficulty
    const diff = CONFIG.DIFFICULTIES[this._selectedDifficulty] || CONFIG.DIFFICULTIES.NORMAL;
  ...
  _applyHero(heroData) {
    const p = this.player;
    const b = heroData.bonuses || {};
    if (b.maxHp)     { p.maxHp = b.maxHp; p.hp = b.maxHp; }
    if (b.armor)     p.armor = b.armor;
    if (b.speed)     p.speed = b.speed;
    if (b.cdMult)    p.cdMult = b.cdMult;
    if (b.dmgMult)   p.dmgMult = b.dmgMult;
    if (b.critChance)p.critChance = b.critChance;
    if (b.xpMult)    p.xpMult = b.xpMult;
    p.heroId = heroData.id;
    // Сохраняем базовые значения для пассивок
    p._baseDmgMult = p.dmgMult;
    p._baseSpeed   = p.speed;
    p._lastKills   = 0;
    p.addWeapon(heroData.startWeapon);
  }
  ...
  _update() {
    ...
    // Castle chest check
    if (this.world) this.world.updateChest(this.player, this);

    // Warrior passive: +1 armor per 5 kills (max +5)
    if (this.player.heroId === 'WARRIOR') {
      const bonusArmor = Math.min(5, Math.floor(this.kills / 5));
      this.player.armor = Math.max(this.player.armor, 2 + bonusArmor);
    }

    // Necromancer passive: heal 0.5 HP per kill (tracked via kills delta)
    if (this.player.heroId === 'NECROMANCER') {
      if (!this._lastKills) this._lastKills = 0;
      const newKills = this.kills - this._lastKills;
      if (newKills > 0) {
        this.player.heal(newKills * 0.5);
        this._lastKills = this.kills;
      }
    }

    // Berserker passive: +50% dmg and speed when HP < 50%
    if (this.player.heroId === 'BERSERKER') {
      const hpPct = this.player.hp / this.player.maxHp;
      if (hpPct < 0.5) {
        if (!this.player._berserkActive) {
          this.player._berserkActive = true;
          this.player.dmgMult  = (this.player._baseDmgMult  || 1.0) * 1.5;
          this.player.speed    = (this.player._baseSpeed    || 230) * 1.5;
        }
      } else {
        if (this.player._berserkActive) {
          this.player._berserkActive = false;
          this.player.dmgMult = this.player._baseDmgMult || 1.0;
          this.player.speed   = this.player._baseSpeed   || 230;
        }
      }
    }
