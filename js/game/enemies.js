...
class EnemySystem {
  ...
  spawn(type, px, py, gameTime, difficulty) {
    ...
    const diff = difficulty || CONFIG.DIFFICULTIES.NORMAL;
    ...
    e.x = pos.x; e.y = pos.y;
    e.maxHp = Math.round(data.hp * Math.pow(scale, 0.6) * diff.enemyHpMult);
    e.hp = e.maxHp;
    e.speed = data.speed * diff.enemySpeedMult;
    e.damage = Math.round(data.damage * Math.pow(scale, 0.4) * diff.enemyDmgMult);
  ...
  update(dt, player, particles, game) {
    ...
    const diff = (game && game._currentDifficulty) || CONFIG.DIFFICULTIES.NORMAL;
    ...
    if (this.spawnTimer <= 0) {
      this.spawnTimer = wave.rate * diff.spawnRateMult;
      const type = Utils.randChoice(wave.types);
      this.spawn(type, player.x, player.y, game.gameTime, diff);
      ...
      if (active < 20 && game.gameTime > 30) {
        for (let i = 0; i < 3; i++) {
          ...
          this.spawn(t, player.x, player.y, game.gameTime, diff);
        }
      }
    }
  }
...
draw(ctx, camX, camY, W, H) {
    ...
    for (const e of this.enemies) {
        ...
        if (burning) { ctx.shadowBlur = 15; ctx.shadowColor = '#ff6600'; }
        if (slowed)  { ctx.shadowBlur = 12; ctx.shadowColor = '#80e8ff'; }
        ...
        if (sprite) {
            ...
            if (!flash) {
                ctx.shadowBlur = burning ? 25 : slowed ? 18 : 12;
                ctx.shadowColor = burning ? '#ff6600' : slowed ? '#80e8ff' : e.data.color;
            }
            const gradient = ctx.createRadialGradient(sx, sy + bob - e.radius * 0.2, 0, sx, sy + bob, e.radius);
            gradient.addColorStop(0, flash ? '#ffffff' : slowed ? '#c0f0ff' : e.data.colorInner);
            gradient.addColorStop(1, flash ? 'rgba(255,200,200,0.8)' : slowed ? 'rgba(100,200,255,0.8)' : e.data.color);
        ...
        // Slow overlay (ice crystals)
        if (slowed) {
            ctx.save();
            const iceOpacity = 0.25 + 0.15 * Math.sin(now * 5);
            ctx.fillStyle = `rgba(100,200,255,${iceOpacity})`;
            ctx.beginPath();
            ctx.arc(sx, sy + bob, e.radius, 0, Math.PI * 2);
            ctx.fill();
            ctx.restore();
