...
class UIManager {
  ...
  _generateChoices(player) {
    ...
    // New weapons (exclude evolutions — they are obtained via evolution system only)
    ...
    const availableWeapons = allWeaponIds.filter(id => {
      if (playerWeaponIds.includes(id)) return false;
      const d = WEAPONS_DATA[id];
      if (d.isEvolution) return false; // evolutions not in level-up pool
      return true;
    });
    ...
    // Pad to 3 if needed — защита от бесконечного цикла
    let padAttempts = 0;
    while (choices.length < POOL_SIZE && padAttempts < 50) {
      padAttempts++;
      const id = Utils.randChoice(allPassiveIds);
      const d = PASSIVES_DATA[id];
      const lvl = player.passives[id] || 0;
      if (lvl >= d.maxLevel) continue;
      // Не дублировать уже добавленные
      if (choices.some(c => c.passiveId === id)) continue;
      choices.push({
        type: 'passive',
        passiveId: id,
        icon: d.icon,
